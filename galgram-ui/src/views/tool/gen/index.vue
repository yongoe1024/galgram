<template>
  <div class="app-container">
    ...
    <el-alert title="导入后，记得添加必要字段：创建人、创建时间、修改人、修改时间、备注等">
    </el-alert>
    <el-table v-loading="loading"
      ...
      <el-table-column label="操作"
        ...
        <template slot-scope="scope">
          <el-button type="text"
                     size="small"
                     @click="handleAddFields(scope.row)"
                     v-hasPermi="['tool:gen:edit']">添加必要字段</el-button>
          <el-button type="text"
                     size="small"
                     icon="el-icon-view"
    ...
    <import-table ref="import"
                  @ok="handleQuery" />
    <create-table ref="create"
                  @ok="handleQuery" />
  </div>
</template>

<script>
import { listTable, previewTable, delTable, genCode, synchDb, addFields } from "@/api/tool/gen"
...
export default {
  ...
  methods: {
    ...
    handleGenTable (row) {
      ...
      if (row.genType === "1") {
        genCode(row.tableName).then(response => {
          this.$modal.msgSuccess("成功生成到serve模块")
        })
      } else {
    ...
    handleAddFields (row) {
      let str = '是否添加: 创建人、创建时间、修改人、修改时间、备注。'
      console.log(row)
      if (row.tplCategory == 'tree') {
        str += '树表关联字段: parentName、parentId、orderNum、ancestors。'
      }
      this.$modal.confirm(str).then(function () {
        return addFields(row.tableId)
      }).then((response) => {
        synchDb(row.tableName)
        this.$modal.confirm('添加成功 ' + response.msg).then(function () {
          this.getList()
        }).catch(() => { })
      }).catch(() => { })
    },
