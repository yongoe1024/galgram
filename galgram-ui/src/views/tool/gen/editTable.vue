...
export default {
  ...
  data () {
    return {
      ...
      info: {
        moduleName: 'project',
        packageName: 'com.galgram.project',
        genType: '1',
    ...
    submitForm () {
      ...
      Promise.all([basicForm, genForm].map(this.getFormPromise)).then(res => {
        ...
        if (validateResult) {
          ...
          let packageName = genTable.packageName
          let moduleName = genTable.moduleName
          //packageName截取最后一个点之后的字符串
          let arr = packageName.split(".")
          let packageName1 = arr[arr.length - 1]
          if (packageName1 != moduleName) {
            this.$modal.msgError("包名末尾与模块名不一致，请重新填写")
            return
          }
          ...
          this.$modal.msgError("缺少必填信息，请重新检查提交内容")
        }
      })
    },
    getFormPromise (form) {
